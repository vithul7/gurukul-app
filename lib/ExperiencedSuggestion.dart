...
class ExperiencedSuggestion extends StatefulWidget {
  const ExperiencedSuggestion({Key? key}) : super(key: key);

  @override
  _ExperiencedSuggestionState createState() => _ExperiencedSuggestionState();
}

class _ExperiencedSuggestionState extends State<ExperiencedSuggestion> {
  User? _user = FirebaseAuth.instance.currentUser;
  InternModel _loggedInUser = InternModel();
  SecondCommentTree tree = SecondCommentTree();
  ...
  TextButton _yesButton() {
    return TextButton(
      ...
      onPressed: () {
        setState(() {
          print("successfully traversed one to the right");
          if (tree.root!.getText()!.toString().endsWith("?")) {
  ...
  Text displaySuggestions() {
    return Text(
      tree.root?.getText() != null ? tree.root!.getText().toString() : "test", style: TextStyle(fontSize: 16)
    );
  }
}
